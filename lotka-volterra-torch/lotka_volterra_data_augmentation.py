...
def sample_squeeze(dist, p, dt, T):
    ...
    return sample, torch.tile(sample, [1, dim2]).reshape(-1, 1)

def param_init(mean, std_init):
    '''
    init params of SDE as log normlas
    '''
    std = torch.log(torch.exp(torch.ones(1) * std_init) - 1)
    param_mean = mean
    param_std = F.softplus(std)
    param_dist = D.log_normal.LogNormal(loc=param_mean, scale=param_std)
...
# approximate.
x_start = D.MultivariateNormal(loc=torch.from_numpy(x0), scale_tril=torch.eye(2) * TAU).sample([P]).unsqueeze(1)
# x_start = []
